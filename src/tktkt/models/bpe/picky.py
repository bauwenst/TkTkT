...
class CacheablePickyBPEArtifacts(PickyBPEArtifacts, CacheableArtifacts):
    ...
    def __init__(self, types: list[str], events: list[Event]):
        super().__init__()
        self._types  = types
        self._events = events
    ...
    def store(self, cache_path: Path):
        ...
        def validate_characters(token: str) -> str:
            for c in token:
                if c.isspace() or len(c.__repr__()) == 6:  # Cannot be printed properly in a text file.
                    raise ValueError(f"Token contains invalid character: {repr(c)}.")  # TODO: Probably should not throw an error when writing a vocabulary!
            return token

        # Vocabulary stores types (not verbose types, because we can reconstruct them from merges)
        self._storeTypes(folder, self._types)
        ...
        with open(folder / CacheablePickyBPEArtifacts._NAME_EVENTS, "w", encoding="utf-8") as f:
            for event in self._events:
                parts = [event.left, event.right]
                if event.is_merge:
                    f.write(f"+ {' '.join(validate_characters(part) for part in parts)}\n")
                else:
                    f.write(f"- {' '.join(validate_characters(part) for part in parts)}\n")
    ...
    def load(cls, cache_path: Path) -> Self:
        ...
        return CacheablePickyBPEArtifacts(
            types=cls._loadTypes(cache_path),
            events=cls._eventsFromFile(cache_path / CacheablePickyBPEArtifacts._NAME_EVENTS)
        )

    ####################################################################################################################

    @classmethod
    def _simpleVocabularyFromNativeFile(cls, path: Path) -> dict[str, int]:
    ...
    @classmethod
    def _verboseVocabularyFromNativeFile(cls, path: Path) -> dict[str, Token]:
        path = Path(path).resolve()
        if path.is_dir():
            path = path / "vocab.json"
...
class PickyBPEVocabulariser(_VocabulariserWithChizhovBackend[CacheablePickyBPEArtifacts]):
    ...
    def _dumpToArtifacts(self, dump_path: Path) -> CacheablePickyBPEArtifacts:
        vocab = CacheablePickyBPEArtifacts._simpleVocabularyFromNativeFile(dump_path)
        events = CacheablePickyBPEArtifacts._eventsFromFile(dump_path)
        return CacheablePickyBPEArtifacts(
            types=sorted(vocab.keys(), key=vocab.get),
            events=events
        )
